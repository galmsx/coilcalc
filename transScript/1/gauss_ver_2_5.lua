...
Diod = read(handle, "*n", "*l")	-- 0- без паралельного диода катушке, 1- с диодом

Vykl = read(handle, "*n", "*l")	-- мм, на каком расстоянии от начального положения пули выключится транзистор катушки
...
mi_addnode(0,(Lkat+Dkat)*-Vol) 				-- ставим точку
mi_addnode(0,(Lkat+Dkat)*Vol)				-- ставим точку
mi_addsegment(0,(Lkat+Dkat)*-Vol,0,(Lkat+Dkat)*Vol)		-- рисуем линию
mi_addarc(0,(Lkat+Dkat)*-Vol,0,(Lkat+Dkat)*Vol,180,max_segm)	-- рисуем дугу

mi_addblocklabel((Lkat+Dkat)*0.7*Vol,0)				-- добавляем блок	
mi_clearselected()						-- отменяем все 
mi_selectlabel((Lkat+Dkat)*0.7*Vol,0)				-- выделяем метку блока
...
T_t={}
Pricina=""
Stop_cikl=0	-- останавливаем цикл если =1
repeat  	------------------------------------------------------------ начинаем цикл
	...
	if Uc < 0 and Diod == 1 then
		Pricina="Uc< 0, напряжение на конденсаторе < 0, Diod == 1 стоит паралельный диод"
		break
	end   
	
	if x > (Lpuli+Lkat-Lsdv) and Force >-1  then 
		Pricina="x > (Lpuli+Lkat-Lsdv) and Force >-1, Пуля вылетела за пределы катушки и сила действующая на неё очень маленькая"
		break
	end 
			
	if x < 0 then 
		Pricina="x < 0, Пуля вылетела назад"
		break
	end 
	...
	if Vel < 0 then 
		Pricina="Vel < 0, скорость отрицательная"
		break
	end 
...
until Stop_cikl==1 -- повторяем расчет

Epuli = (Mpuli*Vel^2)/2 - (Mpuli*Vel0^2)/2
Epuli_all = (Mpuli*Vel^2)/2
EC= (C*(U-Uvykl)^2)/2
KPD = Epuli*100/EC
...
print ("Причина остановки расчета: " .. Pricina)
...
print ("Энергия пули, полученная от этой катушки Дж = " .. Epuli)
print ("Энергия пули Дж = " .. Epuli_all)
...
----------------------------------------------------------------------------------------------------
handle = openfile(File_name .. " V = " .. Vel .. " KPD = " .. KPD .. ".txt", "a")-- создаем файл а - будем дописывать в конец файла w - записать стерев всё что было перед тем

write (handle,"--------------------------------------------------------------------------------\n")
write (handle,"Начало симуляции " .. Start_date,"\n")
write (handle,"Причина остановки расчета: " .. Pricina,"\n")
...
write (handle,"Энергия пули, полученная от этой катушки Дж =" .. Epuli,"\n")
write (handle,"Энергия пули Дж = " .. Epuli_all,"\n")
